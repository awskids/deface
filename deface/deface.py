...
def anonymize_frame(
    dets, frame, mask_scale, replacewith, ellipse, draw_scores, replaceimg
):
    """
    frame is updated
    """
...
def video_detect(
    ...
    for frame in read_iter:
        ...
        if enable_preview:
            cv2.imshow(
                "Preview of anonymization results (quit by pressing Q or Escape)",
                frame[:, :, ::-1],
            )  # RGB -> RGB
...
def parse_cli_args():
    ...
    parser.add_argument(
        "--replaceimg",
        default="../frigate-with-ai/debug/amsa-black.png",
